...
# Load data
data_c = np.load(proc_data_dir+ '/chb01/chb01_01.npy') #Control file
n_epochs, n_channels, n_times = data_c.shape
print(data_c.shape)

# CCM Parameters
np.random.seed(1)
L=n_times   # length of time period
tau=1       # time lag
...
for i in range(limit_channels-1):
    Xhat_My1 = []
    Yhat_Mx1 = []
    ...
    for j in range(i+1, limit_channels):
        ...
        for epoch_idx in range(limit_epochs):
            epoch = data_c[epoch_idx]
            X0 = epoch[i]
            Y0 = epoch[j]
        
            # Apply CCM on each epoch
            ccm_XY = ccm(X0, Y0, tau, E, L) # define ccm object for X -> Y
            ccm_YX = ccm(Y0, X0, tau, E, L) # define ccm object for Y -> X

            Xhat_My2.append(ccm_XY.causality()[0]) 
            Yhat_Mx2.append(ccm_YX.causality()[0])
        
        Xhat_My1.append(Xhat_My2)
        Yhat_Mx1.append(Yhat_Mx2)
