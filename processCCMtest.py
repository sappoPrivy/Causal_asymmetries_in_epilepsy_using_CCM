...
def plot_heatmap(matrix, filename, limit_channels):
    ...
    channel_arr = [f"Ch{i}" for i in range(limit_channels)]
...
def ccm1(subject, proc_data_dir, output_dir, sample_type, filename):
    ...
    plot_convergence(output_filename+f"-Ch({2,3})-convergence", X[2], X[3])
    # compute_ccm(limit_channels,output_filename, X)
...
ccm1("chb01", proc_data_dir, output_dir, "nonses", "sample-1.npz")
