...
ccm.shadow_manifold = shadow_manifold
...
ccm.get_distances = get_distances
# %%add_to ccm
...
ccm.get_nearest_distances = get_nearest_distances
...
ccm.visualize_cross_mapping = visualize_cross_mapping
...
ccm.plot_ccm_correls = plot_ccm_correls

#####################################################
data_c = np.load('my_data.npy') #Control file
# data_p = np.load('patient-ses1-labeltc.npy') #Patient file

X_c= pd.DataFrame(data_c)
# Y_p= pd.DataFrame(data_p)
...
window_size = 5000  # Window length (you can adjust this)
step_size = 5000   # Step size for moving the window
np.random.seed(1)
signal_length = 24104
L=5000
tau=1
E=2
#To select the random chunk
start_index = 1000
window = 24104
...
limit = 2 #len(X_c)  #control
# limit = len(Y_p) #patient
...
#             correl_diffs = correl_xy - correl_yx
#             correl_diffs_1.append(correl_diffs)
    
#             correl_diffs2 = correl_yx - correl_xy
#             correl_diffs_2.append(correl_diffs2)
...
print(np.array(Xhat_My_f).shape)
print(np.array(Yhat_Mx_f).shape)